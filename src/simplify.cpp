...
int main(int argc, char **argv) {
    ...
    const std::string filename =
        (argc > 1) ? argv[1] : CGAL::data_file_path("meshes/cube-meshed.off");
    ...
    // edges drops below 10% of the initial count
    ...
    std::cout << "Time elapsed: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(
                     ...
                     .count()
              << "ms" << std::endl;
    CGAL::IO::write_polygon_mesh((argc > 3) ? argv[3] : filename, surface_mesh,
