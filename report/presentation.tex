...
\newcommand{\mercatorImage}[2][0.8\textwidth]{
  \begin{figure}[H]
    \centering
...
\begin{frame}[fragile]{Methodology: Data acquisition}
  \Large \textbf{Overpass API} and \textbf{cURL} to query \textbf{OpenStreetMap}
...
\begin{frame}[fragile]{Methodology: Data acquisition}
\Large The data will be stored in a \textbf{.json} file:
...
\begin{frame}[fragile]{Methodology: Data acquisition}
\Large Sometimes, multiple \textbf{tags} are missing:
...
\begin{frame}[fragile]{Methodology: Data acquisition}
  \Large \textbf{config.json} to specify the query parameters:
  \begin{lstlisting}[language=json]
  {
...
\begin{frame}{Mercator's projection}
  ...
  \begin{itemize}
    \item \textbf{Most used} projection for representing the Earth's surface on a plane.
    \item \textbf{Conformal} $\Longrightarrow$ preserves angles locally.
    \item \textbf{Distortion} $\Longrightarrow$ relative sizes of countries is not accurate.
  \end{itemize}
\end{frame}
...
\begin{frame}{Mercator's projection}
  ...
  Mathematically, with a point A(latitude, longitude) = A($\phi$, $\lambda$),\\
  \begin{equation}
